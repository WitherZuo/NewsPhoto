...
changed: Sunday, 3rd August 2025
by: Wither Zuo (openworld189@gmail.com)
"""
...
class ZhDate:
    def __init__(self, lunar_year, lunar_month, lunar_day, leap_month=False):
        ...
        if not ZhDate.validate(lunar_year, lunar_month, lunar_day, leap_month):
            raise TypeError(
                f"农历日期不支持所谓“{self}”，超出农历1900年1月1日至2100年12月29日，或日期不存在"
            )
    ...
    def chinese(self):
        ...
        return f"{zh_year}年{zh_month}月{zh_day} {year_tiandi}{shengxiao[(self.lunar_year - 1900) % 12]}年"
    ...
    def __str__(self):
        ...
        Returns:
            str -- 标准格式农历日期字符串
        """
        return f"农历{self.lunar_year}年{'闰' if self.leap_month else ''}{self.lunar_month}月{self.lunar_day}日"
